...
{
  VLOG(2) << fmt::format("Update filter with detection: {}, time: {}, P_M:{}, P_F:{}", detector_output, observation_time, P_M, P_F);
  ...
  if(logLY_ != nullptr)  // If the running sum logLY has been previously initialized ...
  {
  ...
  {
    VLOG(2) << "Initializing lower sum LY.";
    ...
    logLY_ = new double(((detector_output ? gsl_sf_log(P_F) : gsl_sf_log(1 - P_F)) + log1_minus_ST));
  }
