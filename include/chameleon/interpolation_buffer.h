...
struct InterpolationBufferT {
  ...
  //////////////////////////////////////////////////////////////////////////////
  /// \brief Gets the next element in the buffer
  /// \param output The output element
  /// \return True if the function returned an intermediate element, false if we
  ///  had to interpolate
  /// in which case we've reached the end
  ///
  bool GetNext(ElementType& output) {
    // if we have reached the end, interpolate and signal the end
    if (start_index == elements.size()-1) {
      size_t index = start_index;
      output = GetElement(end_time, &index);
      return false;
    } else {
      output = elements[start_index];
      start_index++;
      return true;
    }
    return false;
  ...
  //////////////////////////////////////////////////////////////////////////////
  /// \brief Returns the closest element. Call HasElement before this
  /// function to make sure
  ///        an element exists for this time
  /// \param dTime The time for which we require the element
  /// \param interpolate Interpolate measurements
  /// \return The element
  ///
  ElementType GetElement(const ScalarType time, bool interpolate) const {
    size_t index;
    if(interpolate){
      return GetElement(time, &index);
    }else{
      return GetClosestElement(time, &index);
    }
  }
  ...
  /// \return The element
  ///
  ElementType GetClosestElement(const ScalarType time, size_t* pIndex) const {
  ...
  std::vector<ElementType> GetRange(ScalarType start,
    ...
    // get all the measurements between these two poses, and add them to a
    // vector
    if (HasElement(start)) {
